...
if __name__ == '__main__':
    # 定义参数
    trainingFile = "data/training_v2.txt"
    developFile = "data/develop_v2.txt"
    testingFile = "data/testing_v2.txt"
    resultFile = "test_result/pred_develop.score"
    ...
    with tf.Graph().as_default(), tf.device(gpuDevice):
        ...
        with tf.Session(config=session_conf).as_default() as sess:
            ...
            if os.path.exists(trainedModel + '.index'):
                while True:
                    choice = raw_input("Find pretrained model, restore it ot not?(y/n)")
